...
public class UsuarioController {
	...
	public String formulario(@Valid @ModelAttribute("usuario") Usuario usuario,
		...
		if(result.hasErrors()) {
			...
			if(resultado) {
				model.addAttribute("msgOK", "Usuario creado correctamente");
	...
	//1º hacer jsp con selectores de usuarios y roles
	//2º capturar el usuario y el rol en el controlador de usuario
	//3º buscar usuario y rol por id a la BD
	//4º agregar a la lista de roles del usuario el rol capturado usuario.getRoles().add(Rol)
